...
**v1.4.0 关键变更**：视频壁纸支持 via Cloudflare R2 远程 URL（`img.357561.xyz/wallpaper.mp4` + `img.357561.xyz/image-wallpaper.png`），zip 中**不包含 video/ 目录**。
...
**R2 远程壁纸模式**（v1.4.0+）：壁纸通过 Cloudflare R2（`img.357561.xyz`）托管，zip 中**不需要包含 `video/` 目录**。省掉 zip 体积约 200KB。
...
},
{
  ...
  "name": "可切换壁纸 URL",
  "type": "string",
  "default": "https://img.357561.xyz/image-wallpaper.png",
...
},
{
  ...
  "name": "动态壁纸 URL",
  "type": "string",
  "default": "https://img.357561.xyz/wallpaper.mp4",
...
Glass 当前使用**双壁纸方案**，均托管在 Cloudflare R2（`img.357561.xyz`），不依赖 VPS 本地文件：

| 类型 | URL | 说明 |
|------|-----|:----:|
| 静态底图 | `https://img.357561.xyz/image-wallpaper.png` | 3840×2160 PNG，CSS `background` 加载 |
| 视频壁纸 | `https://img.357561.xyz/wallpaper.mp4` | 动态 MP4，`<video>` 元素叠加，淡入效果 |
...
```html
<div class="bg-layer" id="bg-layer" style="background: #0a0a0f url(https://img.357561.xyz/image-wallpaper.png) center/cover no-repeat;"></div>
<video class="bg-video" id="bg-video" src="https://img.357561.xyz/wallpaper.mp4" muted loop playsinline autoplay preload="auto"></video>
```

**layout.css**（`src/styles/layout.css`）：
```css
.bg-layer { position: fixed; inset: 0; z-index: -1; background: #0a0a0f url(https://img.357561.xyz/image-wallpaper.png) center/cover no-repeat; }
