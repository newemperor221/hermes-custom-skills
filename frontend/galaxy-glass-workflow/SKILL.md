...
```
Cloudflare ─→ cloudflared tunnel ─→ 波兰主控 (<荷兰_IP>:25774) → komari 直接服务（内嵌主题）
                                                                         └→ 读取 SQLite 数据库

本地开发机 (CCS LA <运维本机_IP>) → galaxykit/ 仓库 → build.sh → scp → 波兰主控
```
...
# 2. SCP 到波兰主控
scp -i ~/.ssh/hermes_admin -P 46748 index.html root@<荷兰_IP>:/opt/komari/data/theme/
