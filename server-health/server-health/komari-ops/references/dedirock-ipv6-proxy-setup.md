...
```bash
ssh -p 58193 root@<旧Master_IP> "cat > /tmp/proxy.py << 'PYEOF'
...
```bash
ssh -p 58193 root@<旧Master_IP> "cd /tmp && python3 proxy.py &"
# 或用 nohup
ssh -p 58193 root@<旧Master_IP> "nohup python3 /tmp/proxy.py > /tmp/proxy.log 2>&1 &"
```

### 3. 验证端口监听

```bash
ssh -p 58193 root@<旧Master_IP> "ss -tlnp | grep 25775"
...
```bash
ssh -p 58193 root@<旧Master_IP> "curl -s --connect-timeout 5 http://<洛杉矶2_IP>:25774"
...
| DediRock → 56idc-la | `curl -s http://<洛杉矶2_IP>:25774` | 有响应 |
