...
```bash
ssh -p 52137 root@107.172.231.70 "sqlite3 /opt/komari/data/komari.db 'SELECT username FROM users;'"
```
...
```bash
# 如果用户名是 woioeow
UUID=$(ssh -p 52137 root@107.172.231.70 "sqlite3 /opt/komari/data/komari.db \"SELECT uuid FROM users WHERE username='woioeow';\"")
echo $UUID
```
...
ssh -p 52137 root@107.172.231.70 "sqlite3 /opt/komari/data/komari.db \"UPDATE users SET username='admin' WHERE uuid='$UUID';\""
```
...
```bash
# 安装 bcrypt
ssh -p 52137 root@107.172.231.70 "apt-get update -qq && apt-get install -y python3-pip -qq && pip3 install bcrypt -q"

# 生成哈希
ssh -p 52137 root@107.172.231.70 "python3 -c \"import bcrypt; print(bcrypt.hashpw(b'Komari@2026', bcrypt.gensalt()).decode())\""
```
...
# 2. 上传到服务器
scp -P 52137 /tmp/reset_komari.py root@107.172.231.70:/tmp/reset_komari.py

# 3. 执行
ssh -p 52137 root@107.172.231.70 "python3 /tmp/reset_komari.py"
```
...
```bash
# 检查用户名和哈希前缀
ssh -p 52137 root@107.172.231.70 "sqlite3 /opt/komari/data/komari.db \"SELECT username, substr(passwd,1,30) FROM users;\""
```
