---
description: CDN 缓存 vs Cloudflared Tunnel 404 排查区分，stat.357561.xyz 实际根因记录
---
...
**Cloudflared Tunnel 模式**（stat.357561.xyz 当前）：域名 → Cloudflare Edge → cloudflared tunnel → 源站，**没有 CDN 缓存层**，只有 tunnel 路由。
...
**stat.357561.xyz 是 tunnel 模式**，返回 404 是 tunnel 路由问题，不是缓存问题。

## stat.357561.xyz 404 实际根因（2026-05-10）
...
# 3. 本地 Host header 测试
curl -s -H "Host: stat.357561.xyz" http://127.0.0.1:25774/ | grep -o "video.src = [^;]*"
...
- **stat.357561.xyz 404（2026-05-10）**：误以为是 CDN 缓存，实际是 tunnel token 冲突或 Dashboard 端 tunnel 损坏
