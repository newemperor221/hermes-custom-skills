...
# → 不是直接连 31.58.51.127:45774！

# 检查 agent 进程
sshpass -p '密码' ssh -p PORT root@VPS_IP "pgrep -af agent"
# → /opt/komari/agent -e http://31.58.51.127:45774 -t TOKEN --disable-web-ssh
```
...
sshpass -p '密码' ssh -p PORT root@VPS_IP \
  "curl -s --connect-timeout 3 http://31.58.51.127:45774/api/public | head -5"
# → 成功返回 JSON（HTTP 通畅）
...
```bash
# 检查所有监听端口
sshpass -p 'OX8w$nE9A%tfqb6v' ssh -p 46748 root@31.58.51.127 "ss -tlnp | grep -E '25774|25776|45774'"
...
修改 agent endpoint 为 `http://31.58.51.127:25776`（komari server 直连端口），避开 galaxy-proxy 的 WebSocket 隧道问题。但需要注意端口 25776 的防火墙规则和 cloudflared 隧道路径。
